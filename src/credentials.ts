...
function layerLabel(layer: CredentialLayer): string {
  switch (layer) {
    case 'env':
      return 'env';
    case 'cli':
...
export function parseConfigText(text: string): CredentialFields {
  ...
  if (trimmed.startsWith('{')) {
    try {
      ...
      if (server) applyServer(server, out);
      return out;
    } catch {
...
function applyFlat(obj: Record<string, unknown>, out: CredentialFields): void {
...
function applyServer(server: Record<string, unknown>, out: CredentialFields): void {
...
function applyKey(
  ...
  switch (key) {
    ...
    case 'apikey':
    case 'root_api_key':
      if (!out.apiKey) out.apiKey = value;
      break;
    case 'bearer_token':
    case 'bearer':
      if (!out.bearerToken) out.bearerToken = value;
      break;
    case 'account':
    case 'account_id':
    ...
    case 'user':
    case 'user_id':
      if (!out.user) out.user = value;
      break;
    case 'actor_peer_id':
    case 'peer_id':
      if (!out.peerId) out.peerId = value;
      break;
    default:
      break;
  }
...
export function readConfigFile(path: string): CredentialFields | null {
  try {
...
export function expandHome(value: string | undefined, cwd: string): string | undefined {
  if (!value) return undefined;
  let out = value;
  if (out.startsWith('~/') || out === '~') out = join(homedir(), out.slice(1));
  else if (out.startsWith('./')) out = join(cwd, out.slice(2));
  return out;
}
 ...
 */
export function resolveCredentials(opts: ResolveOptions = {}): CredentialResolution {
  ...
  let mcpUrlOverride: string | null = null;

  const merge = (layer: CredentialLayer, next: CredentialFields | null): void => {
    if (!next) return;
    let any = false;
    for (const key of Object.keys(next) as Array<keyof CredentialFields>) {
  ...
  const cliFile = expandHome(
    env.OPENVIKING_CLI_CONFIG_FILE ?? join(configDir, 'ovcli.conf'),
    cwd,
  );
  const ovFile = expandHome(
    env.OPENVIKING_CONFIG_FILE ?? join(configDir, 'ov.conf'),
    cwd,
  );
  ...
  for (const extra of opts.extraFiles ?? []) {
    ...
    if (p) merge('cli', readConfigFile(p));
  }
  merge('env', {
  ...
  if (env.OPENVIKING_MCP_URL) mcpUrlOverride = env.OPENVIKING_MCP_URL;
...
export function describeLayers(resolution: CredentialResolution): string {
