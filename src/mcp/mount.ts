...
export function buildMcpClientConfig(
  ...
  if (credentials.bearerToken) env.OPENVIKING_BEARER_TOKEN = credentials.bearerToken;
  if (credentials.apiKey) env.OPENVIKING_API_KEY = credentials.apiKey;
  if (credentials.account) env.OPENVIKING_ACCOUNT = credentials.account;
  if (credentials.user) env.OPENVIKING_USER = credentials.user;
  if (settings.pinnedPeerId) env.OPENVIKING_PEER_ID = settings.pinnedPeerId;
