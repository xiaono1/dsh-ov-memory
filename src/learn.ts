...
export function prepareLesson(raw: string): PreparedLesson {
  const trimmed = raw.trim();
  if (!trimmed) throw new Error('memlearn: the lesson is empty — nothing to learn.');
 ...
 */
export async function learnLesson(
  client: OpenVikingClient,
  options: LearnLessonOptions,
): Promise<LearnAttempt> {
  ...
  if (best && (best.score as number) >= options.minScore) {
    await client.writeContent(best.uri, options.lesson, { mode: 'append' }, options.actorPeerId);
    return {
      action: 'merged',
      uri: best.uri,
      score: best.score,
      message: `Appended the lesson to ${best.uri} (score ${(best.score as number).toFixed(2)}); the server re-indexes it in the background.`,
    };
  }

  const top = best
    ? ` (closest hit scored ${(best.score as number).toFixed(2)}, below the ${options.minScore.toFixed(2)} merge floor)`
    : '';
  return {
    action: 'no-match',
    uri: '',
    message:
      `No existing memory is close enough to merge into${top}. OpenViking cannot create ` +
