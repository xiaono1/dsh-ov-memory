...
function mcpName(raw) {
    return `mcp__${MCP_SERVER_NAME}__${raw}`;
}
/** Short display names for the model-facing tool the guard suggests. */
const REMEDY_BY_FAMILY = [
    [/^(fs_)?read|read$/i, 'read'],
    [/^(fs_)?write|^write$|str_replace_editor|edit/i, 'write'],
    [/search|grep/i, 'grep'],
    [/glob/i, 'glob'],
];
export function isOwnMcpTool(name) {
    return name.startsWith(`mcp__${MCP_SERVER_NAME}__`);
}
export function guardVikingUri(call) {
    if (isOwnMcpTool(call.name))
        return { kind: 'allow' };
    if (!LOCAL_PATH_TOOLS.has(call.name))
        return { kind: 'allow' };
    if (!hasVikingTarget(call.arguments))
        return { kind: 'allow' };
    const remedy = REMEDY_BY_FAMILY.find(([pattern]) => pattern.test(call.name))?.[1] ?? 'read';
    return {
        kind: 'deny',
        reason: `"${call.name}" operates on local files, but the call targets a viking:// ` +
...
export function guardMessageFor(decision) {
