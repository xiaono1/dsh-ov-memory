...
export function messageToText(content) {
 ...
 */
export function captureEvent(event, options) {
    ...
    if (type === 'user/message') {
        ...
        if (!isUsableMessage(msg))
            return null;
        if (isOwnInjectedBlock(msg, options.ownPluginSource))
            return null;
    ...
    if (type === 'tool/result' && options.toolResults) {
        ...
        const name = (msg.source && 'callId' in msg.source
